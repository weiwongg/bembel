...
#define __BEMBEL_PIVOTEDCHOLESKY_TOL__ 1e-4
...
int main(int argc, char *argv[]) {
  ...
  if (!setting.compare("david")) {
    interface_geometry =
        Geometry("../geo/refined_cube.bpd", 4.0,
  ...
  VectorXcd int_mean_ref = MatrixXcd::Zero(2*interface_points.rows(), 1);
  VectorXcd int_2mom_ref = MatrixXcd::Zero(2*interface_points.rows(), 1);
  MatrixXcd int_cor_ref = MatrixXcd::Zero(2*interface_points.rows(), 2*interface_points.rows());
  ...
  VectorXcd pot_mean_ref = MatrixXcd::Zero(reference_points.rows(), 1);
  VectorXcd pot_2mom_ref = MatrixXcd::Zero(reference_points.rows(), 1);
  MatrixXcd pot_cor_ref = MatrixXcd::Zero(reference_points.rows(), reference_points.rows());
  ...
  }
  {
    ...
    for (auto l = 0; l <= polynomial_degree_BEM + 1; ++l) {
        VectorXcd int_mean_temp;
        VectorXcd int_2mom_temp;
        MatrixXcd int_cor_temp;

        VectorXcd pot_mean_temp;
        VectorXcd pot_2mom_temp;
        MatrixXcd pot_cor_temp;
        
      std::string identifier =
          std::to_string(refinement_level_BEM) + "_" + std::to_string(l) + "_" +
          std::to_string(interface_lvl) + "_" +
          std::to_string(interface_degree) + "_" + std::to_string(wavenum) +
          "_mc1_" + setting;
      IO::bin2Mat(std::string(refdir) + identifier + "_int_mean.bin",
                  &int_mean_temp);
      IO::bin2Mat(std::string(refdir) + identifier + "_int_2mom.bin",
                  &int_2mom_temp);
      IO::bin2Mat(std::string(refdir) + identifier + "_int_cor.bin",
                  &int_cor_temp);

      IO::bin2Mat(std::string(refdir) + identifier + "_pot_mean.bin",
                  &pot_mean_temp);
      IO::bin2Mat(std::string(refdir) + identifier + "_pot_2mom.bin",
                  &pot_2mom_temp);
      IO::bin2Mat(std::string(refdir) + identifier + "_pot_cor.bin",
                  &pot_cor_temp);

      int_mean_ref += int_mean_temp;
      int_2mom_ref += int_2mom_temp;
      int_cor_ref += int_cor_temp;
      pot_mean_ref += pot_mean_temp;
      pot_2mom_ref += pot_2mom_temp;
      pot_cor_ref += pot_cor_temp;
    }
  }
