...
int main() {
  ...
  for (int polynomial_degree = 0; polynomial_degree < polynomial_degree_max + 1;
    ...
    for (int refinement_level = refinement_level_max;
         refinement_level < refinement_level_max + 1; ++refinement_level) {
      sw.tic();
    ...
    // expected value
    // assert(checkRateOfConvergence(error.tail(2), 2 * polynomial_degree + 3,
    // 0.9));
