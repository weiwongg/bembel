...
int main() {
  ...
  for (auto polynomial_degree : {1, 2, 3}) {
    ...
    for (auto refinement_level : {0, 1, 2, 3}) {
      ...
      // Combine to regularized system
      Rank1Update<H2Matrix<double>, VectorXd> system_matrix(
          disc_op.get_discrete_operator(), one_lf.get_discrete_linear_form());

      // solve system
      ConjugateGradient<Rank1Update<H2Matrix<double>, VectorXd>, Lower | Upper,
