...
int main() {
  ...
  for (int it = 0; it < 4; ++it) {
    Geometry geo("deformed_gamma_" + std::to_string(it) + ".dat", false);

    // The refinement level for the visualization is independent of that of the
    // simulation since one might consider to visualize a coarse discretisation
    ...
    writer.writeToFile("deformed_gamma_" + std::to_string(it) + ".vtp");
  }
  return 0;
}
