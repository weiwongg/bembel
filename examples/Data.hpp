...
/*	@brief This function implements the Laplace fundamental solution,
 * which, if center is placed in the interior domain, satisfies Laplace
 * equation and radiation condition in the exterior domain.
 */
inline double LaplaceFundamentalSolution(
    Eigen::Vector3d pt, Eigen::Vector3d center = Eigen::Vector3d(0, 0, 0)) {
  return 1. / (pt - center).norm();
}
