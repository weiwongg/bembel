...
// information.
#ifndef BEMBEL_SRC_HELMHOLTZ_SINGLELAYERPOTENTIALNORMALDERIVATIVE_HPP_
#define BEMBEL_SRC_HELMHOLTZ_SINGLELAYERPOTENTIALNORMALDERIVATIVE_HPP_

namespace Bembel {
// forward declaration of class HelmholtzSingleLayerPotentialNormalDerivative in
// order to define traits
template <typename LinOp>
class HelmholtzSingleLayerPotentialNormalDerivative;

template <typename LinOp>
struct PotentialTraits<HelmholtzSingleLayerPotentialNormalDerivative<LinOp>> {
...
template <typename LinOp>
class HelmholtzSingleLayerPotentialNormalDerivative
    : public PotentialBase<HelmholtzSingleLayerPotentialNormalDerivative<LinOp>,
                           LinOp> {
  // implementation of the kernel evaluation, which may be based on the
  // information available from the superSpace
 public:
  HelmholtzSingleLayerPotentialNormalDerivative() {}
  ...
  evaluateIntegrand_impl(const FunctionEvaluator<LinOp> &fun_ev,
    ...
    // evaluate kernel
    auto kernel = evaluateKernelNormalDerivative(point, x_f);
   ...
   */
  std::complex<double> evaluateKernelNormalDerivative(
      const Eigen::Vector3d &x, const Eigen::Vector3d &y) const {
    auto c = x - y;
    auto r = c.norm();
    auto r3 = r * r * r;
    auto i = std::complex<double>(0., 1.);
    return x.dot(c) / x.norm() *
           (std::exp(-i * wavenumber_ * r) * (-1. - i * wavenumber_ * r) / r3) /
           4. / BEMBEL_PI;
  }
  //////////////////////////////////////////////////////////////////////////////
  //    setters
