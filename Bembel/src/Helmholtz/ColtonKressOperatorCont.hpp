...
// information.
//
#ifndef BEMBEL_LINEAROPERATOR_HELMHOLTZ_HELMHOLTZCOLTONkRESSOPERATORCONT_H_
#define BEMBEL_LINEAROPERATOR_HELMHOLTZ_HELMHOLTZCOLTONkRESSOPERATORCONT_H_

namespace Bembel {
// forward declaration of class HelmholtzColtonKressOperatorCont in order to define
// traits
class HelmholtzColtonKressOperatorCont;

template <>
struct LinearOperatorTraits<HelmholtzColtonKressOperatorCont> {
 ...
 */
class HelmholtzColtonKressOperatorCont
    : public LinearOperatorBase<HelmholtzColtonKressOperatorCont> {
  // implementation of the kernel evaluation, which may be based on the
  // information available from the superSpace
 public:
  HelmholtzColtonKressOperatorCont() {}
  template <class T>
  void evaluateIntegrand_impl(
      const T &super_space, const SurfacePoint &p1, const SurfacePoint &p2,
      Eigen::Matrix<
          typename LinearOperatorTraits<HelmholtzColtonKressOperatorCont>::Scalar,
    ...
    auto integrand =
        evaluateKernelGrad(x_f, y_f, x_n) * x_kappa * y_kappa * ws * wt;
  ...
  Eigen::Matrix<std::complex<double>, 1, 1> evaluateFMMInterpolation_impl(
    ...
    intval(0) = evaluateKernelGrad(x_f, y_f, x_n) * x_kappa * y_kappa;
  ...
  std::complex<double> evaluateKernelGrad(const Eigen::Vector3d &x,
    ...
    return std::exp(-i * wavenumber_ * r) *
           (-c.dot(y_n) * (1. + i * wavenumber_ * r) / r3 -
            0.5 * i * wavenumber_ / r) /
           4. / BEMBEL_PI;
  }
  //////////////////////////////////////////////////////////////////////////////
