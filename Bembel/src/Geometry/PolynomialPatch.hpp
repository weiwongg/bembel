...
public:
  ...
  evalJacobian(const Eigen::Vector2d &reference_point) const {
    Eigen::Matrix<double, 3, Eigen::Dynamic> eval_buffer(3, 2 * (py_ + 1));
  ...
  evalNormal(const Eigen::Vector2d &reference_point) const {
...
private:
  ...
  template <typename Derived, typename otherDerived>
  Eigen::Matrix<double, 3, 2>
  HornersMethodDerivative(const Eigen::MatrixBase<Derived> &coeffs,
                          const Eigen::MatrixBase<otherDerived> &x,
                          double xi) const {
    Eigen::Matrix<double, 3, 2> retval;
