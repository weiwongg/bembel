...
// information.
#ifndef BEMBEL_GEOMETRY_NURBSPATCH_H_
#define BEMBEL_GEOMETRY_NURBSPATCH_H_
namespace Bembel {
...
class NURBSPatch {
 public:
  ...
  //////////////////////////////////////////////////////////////////////////////
  NURBSPatch(){};
  NURBSPatch(const std::vector<Eigen::Matrix<double, -1, -1>> &control_points,
             const std::vector<double> &knots_x,
             const std::vector<double> &knots_y) {
    init_NURBSPatch(control_points, knots_x, knots_y);
  }
  //////////////////////////////////////////////////////////////////////////////
  /// init
  //////////////////////////////////////////////////////////////////////////////
  inline void init_NURBSPatch(
...
inline std::vector<NURBSPatch> PatchShredder(const NURBSPatch &patch) noexcept {
  ...
  std::vector<NURBSPatch> out(xchips * ychips);
...
// Shredds a whole vector of NURBSPatches
inline std::vector<NURBSPatch> PatchShredder(
    const std::vector<NURBSPatch> &patches) noexcept {
  std::vector<NURBSPatch> out;
  const int input_size = patches.size();

  for (int i = 0; i < input_size; i++) {
    std::vector<NURBSPatch> tmp = PatchShredder(patches[i]);
