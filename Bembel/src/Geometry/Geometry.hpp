...
class Geometry {
 public:
  ...
  Geometry(const std::string &filename, double scale = 1.,
    ...
    init_Geometry(filename, scale, shift);
  }
  Geometry(const std::vector<Eigen::MatrixXd> &PE, double scale = 1.,
           Eigen::Vector3d shift = Eigen::VectorXd::Zero(3)) {
    geometry_ = std::make_shared<PatchVector>();
    *geometry_ =
        init_PatchVectorFromPoints<PatchVector::value_type>(PE, scale, shift);
  }
  ...
  inline void init_Geometry(const std::string &filename, double scale = 1.,
    ...
    geometry_ = std::make_shared<PatchVector>();
    *geometry_ =
        init_PatchVector<PatchVector::value_type>(filename, scale, shift);
  }
  //////////////////////////////////////////////////////////////////////////////
  //    getters
  //////////////////////////////////////////////////////////////////////////////
